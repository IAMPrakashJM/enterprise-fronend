...
try{
 ...
 const child=spawn(application,[],{env:{...process.env,NEXORA_SENTINEL_TEST_PANIC_ONCE:trigger},stdio:'ignore'});
 const deadline=setTimeout(()=>child.kill('SIGKILL'),30000);
 const [code]=await once(child,'exit');clearTimeout(deadline);assert.equal(code,86,'Debug panic hook must run and exit the native process');
