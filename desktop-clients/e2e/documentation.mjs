...
try {
 ...
 await center.getByRole('button',{name:'Patches',exact:true}).click();await center.getByText('No published patches for this version.',{exact:true}).waitFor();
 ...
 assert.deepEqual(errors,[]);console.log('PASS documentation: page guide, release archive, patch empty state, exact-section notice, durable read state Arabic, Hindi and Malayalam');
