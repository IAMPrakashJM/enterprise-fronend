        ...
        <Card data-tour="chart" className="min-w-0">
          ...
          <CardContent className="pb-3 pt-2">
            ...
            <div className="mt-1 grid grid-cols-2 gap-2 border-t border-[var(--border)] pt-3 sm:grid-cols-4">
              {["Forecast confidence", "Data completeness", "Plan attainment", "Exception rate"].map((label, index) => <div key={label} className="rounded-xl bg-[var(--surface-2)] px-3 py-2"><div className="text-[length:calc(8.5px*var(--fs-scale))] font-bold uppercase tracking-[.08em] text-[var(--text-subtle)]">{label}</div><div className="mt-1 flex items-end justify-between"><span className="text-[length:calc(14px*var(--fs-scale))] font-black">{[94, 99, 91, 3.8][index]}{index === 3 ? "%" : "%"}</span><span className="text-[length:calc(8px*var(--fs-scale))] font-bold text-[var(--success-ink)]">healthy</span></div></div>)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="min-w-0 overflow-hidden">
          ...
          <div className="nex-scrollbar overflow-x-auto">
            <table className="w-full min-w-[680px] border-collapse text-left">
              ...
              <tbody>{branchRows.map((row) => <tr key={row.branch} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-2)]"><td className="px-4 py-3 text-[length:calc(10.5px*var(--fs-scale))] font-extrabold">{row.branch}</td><td className="px-4 py-3 text-[length:calc(10px*var(--fs-scale))] font-bold">{row.volume}</td><td className="px-4 py-3"><div className="flex items-center gap-2"><div className="h-1.5 w-20 overflow-hidden rounded-full bg-[var(--surface-3)]"><div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${Math.min(row.plan, 100)}%` }} /></div><span className="text-[length:calc(9px*var(--fs-scale))] font-extrabold">{row.plan}%</span></div></td><td className={cn("px-4 py-3 text-[length:calc(9.5px*var(--fs-scale))] font-extrabold", row.variance.startsWith("+") ? "text-[var(--success-ink)]" : "text-[var(--danger-ink)]")}>{row.variance}</td><td className="px-4 py-3 text-[length:calc(10px*var(--fs-scale))] font-bold">{row.exceptions}</td><td className="px-4 py-3"><Badge tone={row.health === "Strong" ? "success" : row.health === "Stable" ? "info" : "warning"}>{row.health}</Badge></td></tr>)}</tbody>
            </table>
          </div>
        </Card>

        <Card>
          <CardHeader><CardTitle title="Live activity" subtitle="Automations, approvals and exceptions" action={<span className="flex items-center gap-1 text-[length:calc(8.5px*var(--fs-scale))] font-bold text-[var(--success-ink)]"><span className="size-1.5 animate-pulse rounded-full bg-current" />streaming</span>} /></CardHeader>
