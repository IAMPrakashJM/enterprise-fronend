      ...
      <Card className="min-h-[590px] overflow-hidden">
        ...
        <div className={cn("flex min-h-[540px]", preferences.formNavigation === "rail" && "lg:flex-row")}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="p-5">
              ...
              <div className="mt-6 grid gap-3 lg:grid-cols-3">
                <div className="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-[var(--success-ink)]" /><div><div className="text-[length:calc(10px*var(--fs-scale))] font-extrabold">Policy-aware</div><div className="mt-1 text-[length:calc(9px*var(--fs-scale))] leading-relaxed text-[var(--text-muted)]">Visible and editable fields can be controlled by role, branch and record state.</div></div></div>
                ...
                <div className="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3"><LockKeyhole className="mt-0.5 size-4 shrink-0 text-[var(--warning-ink)]" /><div><div className="text-[length:calc(10px*var(--fs-scale))] font-extrabold">Audit protected</div><div className="mt-1 text-[length:calc(9px*var(--fs-scale))] leading-relaxed text-[var(--text-muted)]">Sensitive changes should be versioned and submitted with a reason.</div></div></div>
              </div>
            </div>
          </div>
          ...
          <aside className="hidden w-64 shrink-0 border-l border-[var(--border)] bg-[var(--surface-2)] p-4 2xl:block">
            ...
            <div className="mt-3 space-y-2">{schema.sections.map((item, index) => <button key={item.id} type="button" onClick={() => setActiveSection(item.id)} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-[var(--surface)]"><span className={cn("flex size-5 items-center justify-center rounded-md text-[length:calc(8px*var(--fs-scale))] font-black", item.id === activeSection ? "bg-[var(--primary-fill)] text-white" : "bg-[var(--surface-3)]")}>{index + 1}</span><span className="min-w-0 flex-1 truncate text-[length:calc(9.5px*var(--fs-scale))] font-bold">{item.title}</span><span className="text-[length:calc(8.5px*var(--fs-scale))] font-bold text-[var(--text-muted)]">{completion[item.id]}%</span></button>)}</div>
