...
function ScheduleModal({ open, onClose, reportTitle }: { open: boolean; onClose: () => void; reportTitle: string }) {
  ...
  return <Modal open={open} onClose={onClose} title="ui.schedule.report.delivery.e7dd43ec" subtitle="ui.run.the.report.asynchronously.and.deliver.it.by.email.bd54a2b2" size="md" footer={<><Button variant="ghost" onClick={onClose}><LocalizedText message="ui.cancel.19766ed6" /></Button><Button variant="primary" leftIcon={<Send className="size-3.5" />} disabled><LocalizedText message="ui.create.schedule.5b08f3c7" /></Button></>}><p role="status" className="px-5 pt-4 text-sm"><LocalizedText message="Scheduled delivery is not connected. No report will be sent." /></p><div className="grid gap-4 p-5 md:grid-cols-2"><Input className="md:col-span-2" label="ui.schedule.name.60918e4f" defaultValue={t("Monthly {report}", {report:t(reportTitle)})} required /><Select label="ui.frequency.16b6668d" value="monthly" options={[{ label: "Daily", value: "daily" }, { label: "Weekly", value: "weekly" }, { label: "Monthly", value: "monthly" }, { label: "Quarterly", value: "quarterly" }]} onChange={() => undefined} /><TimeInput label="ui.delivery.time.2898c4bc" defaultValue="07:30" /><DateInput label="ui.start.date.81696931" defaultValue="2026-10-01" /><Select label="ui.time.zone.b9fe1464" value="dubai" options={[{ label: "Asia/Dubai (UTC+4)", value: "dubai" }, { label: "Asia/Kolkata (UTC+5:30)", value: "kolkata" }]} onChange={() => undefined} /><MultiSelect className="md:col-span-2" label="ui.recipients.1e8568a8" value={recipients} onChange={setRecipients} options={[{ label: "Prakash Mathew • prakash@nexora.example", value: "prakash@nexora.example" }, { label: "Finance Leadership", value: "finance-leadership@nexora.example" }, { label: "Branch Managers", value: "branch-managers@nexora.example" }]} /><Select label="ui.output.format.93c53d37" value="xlsx" options={[{ label: "Excel workbook", value: "xlsx" }, { label: "PDF", value: "pdf" }, { label: "CSV", value: "csv" }]} onChange={() => undefined} /><Select label="ui.delivery.mode.c9585346" value="attachment" options={[{ label: "Email attachment", value: "attachment" }, { label: "Secure download link", value: "link" }, { label: "Both", value: "both" }]} onChange={() => undefined} /><Toggle className="md:col-span-2" label="ui.send.only.when.data.exists.8bac5dec" description="ui.skip.delivery.when.the.filtered.result.is.empty.8d7b6625" checked={true} onChange={() => undefined} /></div></Modal>;
}
