...
export function ConfiguredTemplatePage({page,target}: {page:PageDefinition;target:NavigationTarget}) {
 const adapter=useContext(ProductPageTemplateContext),{user}=useSession(),product=useProduct(),{preferences}=useERP();
 ...
 return <ConnectedEditor key={JSON.stringify(scope)} scope={scope} definition={definition} adapter={adapter} preferences={preferences} readOnly={target.mode==='view'||!canProductAction(product,target.mode==='new'?'create':'edit')}/>;
}
function ConnectedEditor({scope,definition,adapter,preferences,readOnly}: {scope:TemplateScope;definition:PageTemplateDefinition;adapter:PageTemplateAdapter;preferences:React.ComponentProps<typeof PageTemplateWorkspace>['preferences'];readOnly:boolean}) {
 ...
 return <PageTemplateWorkspace definition={definition} scope={{...scope,recordId:document.id}} initialDocument={document} adapter={adapter} preferences={preferences} readOnly={readOnly}/>;
}
