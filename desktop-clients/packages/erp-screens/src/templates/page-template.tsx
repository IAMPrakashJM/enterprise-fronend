...
import {Card,CardHeader,CardTitle,CardContent,CardGrid,Button,Badge,Input,Textarea,Tabs,DescriptionList,EmptyState,LoadingState,AccessDenied,ConfirmDialog,RecoveryNotice,failureFromError,useLocalization} from '@pepbits/ops-ui';
import {DEFAULT_PREFERENCES,createFormatters,type PageTemplateDefinition,type TemplateDocument,type TemplateLayout,type UserPreferences} from '@pepbits/erp-config';
...
export interface PageTemplateProps {definition:PageTemplateDefinition;scope:TemplateScope;initialDocument:TemplateDocument;adapter?:PageTemplateAdapter;preferences?:UserPreferences;layout?:TemplateLayout;readOnly?:boolean;state?:'ready'|'loading'|'empty'|'denied';onSaved?:(document:TemplateDocument)=>void;support?:React.ReactNode}
...
export function PageTemplateWorkspace(props:PageTemplateProps) {
 return <TemplateEditor key={JSON.stringify([props.scope,props.definition.id])} {...props}/>;
}
function TemplateEditor({definition,scope,initialDocument,adapter,preferences=DEFAULT_PREFERENCES,layout,readOnly=false,state='ready',onSaved,support}:PageTemplateProps) {
 ...
 const effectiveLayout=layout??preferences.formNavigation;
 ...
 const props:TemplateEngineProps={definition,document,update,disabled,errors,format,density:preferences.density,precision:preferences.decimalPlaces,pageSize:preferences.pageSize,resultView:preferences.resultView};
 const fields=(all=false)=><div className={effectiveLayout==='rail'&&!all?'grid gap-4 xl:grid-cols-[180px_minmax(0,1fr)]':'space-y-4'}>
  ...
  <div className="space-y-3">{definition.sections.filter(item=>all||item.id===section).map(item=><TemplateFields key={item.id} section={item} values={document.values} errors={errors} disabled={disabled} onChange={(id,value)=>update({...document,values:{...document.values,[id]:value}})}/>)}
   {!all&&effectiveLayout==='wizard'?<div className="flex justify-between gap-2"><Button disabled={definition.sections.findIndex(item=>item.id===section)===0} onClick={()=>setSection(definition.sections[definition.sections.findIndex(item=>item.id===section)-1].id)}>{t('template.previous')}</Button><Button disabled={definition.sections.findIndex(item=>item.id===section)===definition.sections.length-1} onClick={()=>setSection(definition.sections[definition.sections.findIndex(item=>item.id===section)+1].id)}>{t('template.next')}</Button></div>:null}
  </div>
 </div>;
 ...
 return <section className="space-y-4" data-page-template={definition.id} data-template-engine={definition.engine} style={{'--fs-scale':'var(--fs-form)'} as React.CSSProperties}>
  ...
  {state==='loading'?<LoadingState/>:state==='denied'?<AccessDenied/>:state==='empty'?<EmptyState/>:<CardGrid className="items-start gap-4 2xl:grid-cols-[minmax(0,1fr)_260px]"><div className="min-w-0" data-tour="template-body">{body}</div><aside className="space-y-4"><TemplateSummary document={document}/>{support??<Card><CardHeader><CardTitle title="template.integration"/></CardHeader><CardContent className="space-y-2 text-sm text-[var(--text-muted)]"><p>{t('template.adapterHelp')}</p><p>{t('template.preferenceHelp')}</p></CardContent></Card>}</aside></CardGrid>}
 </section>;
}
