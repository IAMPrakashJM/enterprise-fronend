...
vi.mock('@pepbits/auth',()=>({useSession:()=>({user:{id:'user',tenantId:'tenant'}}),readToken:()=> 'token',authedFetch:vi.fn(),reportOperationFailure:vi.fn(),reportSentinelFailure:vi.fn()}));
...
function setup(job:ImportJob){const adapter={latest:vi.fn().mockResolvedValue(job),preview:vi.fn(),run:vi.fn()};const onImported=vi.fn();render(<ProductServicesProvider services={{imports:adapter,drafts}}><CsvImportDialog open onClose={vi.fn()} page={PAGE_REGISTRY['customer-master']} productId="test" onImported={onImported}/></ProductServicesProvider>);return {adapter,onImported};}
