    ...
    <div className="flex w-full flex-col gap-3">
      ...
      <div className={cn("grid min-h-[610px] gap-3", preferences.billingLayout === "split" ? "xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,.65fr)]" : "xl:grid-cols-[minmax(0,1fr)_310px]")}>
        ...
        <div className="space-y-3">
          <Card data-tour="bill-summary" className="overflow-hidden"><div className="border-b border-[var(--border)] bg-[var(--surface-2)] px-4 py-3"><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-xl bg-[var(--primary-soft)] text-[var(--primary)]"><CircleDollarSign className="size-4" /></span><div><div className="text-[length:calc(11px*var(--fs-scale))] font-black">Invoice summary</div><div className="text-[length:calc(8.5px*var(--fs-scale))] text-[var(--text-muted)]">{lines.length} line items • AED</div></div></div></div><div className="p-4">{[["Gross subtotal", totals.subtotal], ["Line discounts", -totals.discount], ["Taxable amount", totals.taxable], ["VAT", totals.tax]].map(([label, value]) => <div key={String(label)} className="flex items-center justify-between border-b border-[var(--border)] py-2.5 text-[length:calc(9.5px*var(--fs-scale))]"><span className="text-[var(--text-muted)]">{label}</span><span className="font-extrabold tabular-nums">{format.money(Number(value))}</span></div>)}<div className="mt-3 rounded-xl bg-[var(--primary)] p-3 text-white"><div className="text-[length:calc(8.5px*var(--fs-scale))] font-bold uppercase tracking-[.1em] opacity-75">Invoice total</div><div className="mt-1 text-[length:calc(22px*var(--fs-scale))] font-black tracking-[-.04em]">{format.money(totals.total)}</div><div className="mt-2 flex justify-between text-[length:calc(8.5px*var(--fs-scale))] opacity-80"><span>Balance after payments</span><span className="font-bold">{format.money(Math.max(0, totals.total - 35000))}</span></div></div></div></Card>
