...
export function DcpRuntimeFields({fields,values,view,change,disabled=false,prefix=''}:{fields:DcpRuntimeField[];values:Record<string,DcpValue|undefined>;view:DcpRuntimeView;change:(code:string,value:DcpValue)=>void;disabled?:boolean;prefix?:string}){
 const {t}=useLocalization();
 return <CardGrid columns={2}>{fields.map(f=>{
  ...
  if(f.type==='COLLECTION'){
   ...
   return <Card key={f.code} className="col-span-full"><div tabIndex={-1} data-dcp-collection={path}><CardHeader><CardTitle title={f.label}/></CardHeader><CardContent className="space-y-3">{error&&<p role="alert">{error}</p>}{rows.map((row,i)=>{
    ...
    return <Card key={row._id??String(row._localKey??i)}><CardContent className="space-y-2">{allowed?<DcpRuntimeFields fields={allowed} values={row} view={view} prefix={rowPath+'.'} disabled={off} change={(id,v)=>change(f.code,rows.map((r,j)=>j===i?{...r,[id]:v}:r))}/>:<p role="alert">{t('designer.v1.rowUnavailable')}</p>}
    <Button disabled={off||!allowed} onClick={()=>change(f.code,row._id?rows.map((r,j)=>j===i?{...r,_delete:true}:r):rows.filter((_,j)=>i!==j))}>{t('designer.v1.removeRow')}</Button></CardContent></Card>;
