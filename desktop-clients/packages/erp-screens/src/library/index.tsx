    ...
    <div className="flex w-full flex-col gap-3">
      ...
      <div className="grid gap-3 xl:grid-cols-[1.4fr_.8fr]">
        <Card>
          ...
          <div className="overflow-auto"><table className="w-full min-w-[680px] text-[length:calc(10px*var(--fs-scale))]"><thead className="bg-[var(--surface-2)] text-left text-[length:calc(8.5px*var(--fs-scale))] uppercase tracking-[.08em] text-[var(--text-subtle)]"><tr><th className="px-4 py-2.5">Component</th><th className="px-4 py-2.5">Source</th><th className="px-4 py-2.5">Contract</th><th className="px-4 py-2.5">Status</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{COMPONENT_ROWS.map(([name, source, contract]) => <tr key={name} className="hover:bg-[var(--surface-2)]"><td className="px-4 py-3 font-extrabold">{name}</td><td className="px-4 py-3 font-mono text-[length:calc(9px*var(--fs-scale))] text-[var(--primary)]">{source}</td><td className="px-4 py-3 text-[var(--text-muted)]">{contract}</td><td className="px-4 py-3"><Badge tone="success">Ready</Badge></td></tr>)}</tbody></table></div>
        </Card>
        <Card>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        ].map(([title, description], index) => <Card key={title}><CardContent><span className="flex size-8 items-center justify-center rounded-xl bg-[var(--primary-soft)] text-[length:calc(10px*var(--fs-scale))] font-black text-[var(--primary)]">0{index + 1}</span><h3 className="mt-3 text-[length:calc(12px*var(--fs-scale))] font-black">{title}</h3><p className="mt-1.5 text-[length:calc(9.5px*var(--fs-scale))] leading-relaxed text-[var(--text-muted)]">{description}</p></CardContent></Card>)}
      </div>
    </div>
  );
}
