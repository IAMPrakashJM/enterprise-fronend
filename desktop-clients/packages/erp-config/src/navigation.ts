...
for(const page of CLINICAL_TEMPLATE_PAGES)PAGE_REGISTRY[page.id]={id:page.id,title:({'allyvora-patient-query':'Patient Query','allyvora-patient-record':'Patient Record','allyvora-patient-360':'360 Data','billing-clinic':'Billing Clinic','clinical-triage':'Clinical Triage'} as Record<string,string>)[page.id],titleKey:page.title,subtitle:'template.clinical.demoNotice',kind:'library',module:'library',icon:BookOpen};
