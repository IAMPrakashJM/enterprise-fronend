  ...
  return (
    <div className="mx-auto flex max-w-[1600px] flex-col gap-3">
      ...
      <Card className="min-h-[590px] overflow-hidden">
        {preferences.formNavigation !== "rail" ? <FormNavigationControl type={preferences.formNavigation} sections={schema.sections} activeId={activeSection} onChange={setActiveSection} completion={completion} /> : null}
        <div className={cn("flex min-h-[540px]", preferences.formNavigation === "rail" && "lg:flex-row")}>
          {preferences.formNavigation === "rail" ? <FormNavigationControl type="rail" sections={schema.sections} activeId={activeSection} onChange={setActiveSection} completion={completion} /> : null}
          <div className="min-w-0 flex-1">
            <div className="border-b border-[var(--border)] bg-[var(--surface)] px-5 py-4">
              <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className="flex size-8 items-center justify-center rounded-xl bg-[var(--primary-soft)] text-[11px] font-black text-[var(--primary-strong)]">{activeIndex + 1}</span><div><h2 className="text-[14px] font-black tracking-[-.02em]">{section.title}</h2><p className="mt-0.5 text-[10px] text-[var(--text-muted)]">{section.description}</p></div></div></div><div className="hidden items-center gap-2 lg:flex"><Badge tone="neutral">{section.fields.length} fields</Badge><Badge tone={completion[section.id] === 100 ? "success" : "warning"}>{completion[section.id]}% complete</Badge></div></div>
            </div>

            <div className="p-5">
              <div className="grid max-w-5xl gap-x-5 gap-y-4 md:grid-cols-2">
              ...
              <div className="mt-6 grid gap-3 lg:grid-cols-3">
                <div className="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-[var(--success)]" /><div><div className="text-[10px] font-extrabold">Policy-aware</div><div className="mt-1 text-[9px] leading-relaxed text-[var(--text-muted)]">Visible and editable fields can be controlled by role, branch and record state.</div></div></div>
                <div className="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3"><Sparkles className="mt-0.5 size-4 shrink-0 text-[var(--primary)]" /><div><div className="text-[10px] font-extrabold">Schema-driven</div><div className="mt-1 text-[9px] leading-relaxed text-[var(--text-muted)]">Rail, tabs and wizard render the same section and field definitions.</div></div></div>
                <div className="flex items-start gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] p-3"><LockKeyhole className="mt-0.5 size-4 shrink-0 text-[var(--warning)]" /><div><div className="text-[10px] font-extrabold">Audit protected</div><div className="mt-1 text-[9px] leading-relaxed text-[var(--text-muted)]">Sensitive changes should be versioned and submitted with a reason.</div></div></div>
              </div>
            </div>
          </div>

          <aside className="hidden w-64 shrink-0 border-l border-[var(--border)] bg-[var(--surface-2)] p-4 2xl:block">
            <div className="text-[9px] font-black uppercase tracking-[.12em] text-[var(--text-subtle)]">Record summary</div>
            <div className="mt-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3"><div className="text-[8.5px] font-bold uppercase tracking-[.08em] text-[var(--text-subtle)]">Overall completion</div><div className="mt-2 flex items-end justify-between"><span className="text-[22px] font-black tracking-[-.04em]">{Math.round(Object.values(completion).reduce((sum, item) => sum + item, 0) / schema.sections.length)}%</span><Badge tone="brand">{schema.sections.length} sections</Badge></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[var(--surface-3)]"><div className="h-full rounded-full bg-[var(--primary)]" style={{ width: `${Math.round(Object.values(completion).reduce((sum, item) => sum + item, 0) / schema.sections.length)}%` }} /></div></div>
            <div className="mt-3 space-y-2">{schema.sections.map((item, index) => <button key={item.id} type="button" onClick={() => setActiveSection(item.id)} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left hover:bg-[var(--surface)]"><span className={cn("flex size-5 items-center justify-center rounded-md text-[8px] font-black", item.id === activeSection ? "bg-[var(--primary)] text-white" : "bg-[var(--surface-3)]")}>{index + 1}</span><span className="min-w-0 flex-1 truncate text-[9.5px] font-bold">{item.title}</span><span className="text-[8.5px] font-bold text-[var(--text-muted)]">{completion[item.id]}%</span></button>)}</div>
            <div className="mt-4 rounded-xl border border-dashed border-[var(--border-strong)] p-3 text-[9px] leading-relaxed text-[var(--text-muted)]"><b className="text-[var(--text)]">Form mode:</b> {preferences.formNavigation}. Change it in My Preferences; the record schema and data remain unchanged.</div>
          </aside>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-[var(--border)] bg-[var(--surface-2)] px-4 py-3">
          ...
          <div className="text-[9px] font-semibold text-[var(--text-muted)]">Section {activeIndex + 1} of {schema.sections.length} • {preferences.formNavigation} navigation</div>
