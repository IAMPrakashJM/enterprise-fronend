...
export function createRecordStore(file) {
  let data = {};
  ...
  return {
    ...
    handle(user, key, action, body) {
      ...
      if (action === 'load') return { status: 200, body: result(entry, user.id) };
      ...
      if (action === 'create') {
        ...
        const snapshot = { values: body.values, baseVersion: body.baseVersion, version, savedAt };
