...
| --- | --- | --- | --- |
| Named templates | 97 | PageTemplateWorkspace and 12 engine families | Definitions render; supported tables/layout/page-size use managed preferences; adapter behavior retains validation/recovery |
...
| Clinical pages | 3 | PatientQueryTemplate, PatientRecordTemplate, Patient360Template | API data/metadata, fields/actions, locks, formatting, shortcuts, previews, errors and relevant scope |
...
Total: 129 unique destinations. The [inventory](inventory.json) names each page and engine; the [original audit inventory](../../../desktop-clients/docs/library-preference-audit.md) preserves the pre-correction findings.

The 97 named templates use master, list, order, booking, case, result, finance, reconcile, report, dashboard, entity and admin engines. A test of one shared renderer provides relevant coverage to its consumers, but does not prove every application's domain rules.
