...
Open **Library → Page Library → List of pages**, the first item in this sidebar group. The catalog lists every other page in that group that is available to the signed-in user. The current group contains 14 working pages: Worklist Query, Master Record – Main, 360 Data, Billing Clinic, Clinical Triage, Clinical Consultation, OP Consultation, Comprehensive Consultation, OP Registration, Emergency Registration, Inpatient Admission, Consultation Entry, Consultation Entry v2 and DCP Visual Designer. The consultation reference pages use `consultation-entry-design` and `consultation-entry-v2`; the existing application route `consultation-entry` is preserved.
